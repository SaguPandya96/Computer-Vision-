...
namespace CSImageViewer {
    ...
    public class CSImageViewer : Form {
        ...
        private MenuItem menuItemBinary;
        private MenuItem menuItemIsBinary;
        private MenuItem menuItemIsBorderEmpty;
        private MenuItem menuItemIs4Connected;
        ...
        private void InitializeComponent() {
            ...
            this.menuItemBinary = new System.Windows.Forms.MenuItem();
            this.menuItemIsBinary = new System.Windows.Forms.MenuItem();
            this.menuItemIsBorderEmpty = new System.Windows.Forms.MenuItem();
            this.menuItemIs4Connected = new System.Windows.Forms.MenuItem();
            this.SuspendLayout();
            // 
            // mainMenu1
            // 
            ...
            // menuItemBinary
            // 
            this.menuItemBinary.Index = 2;
            this.menuItemBinary.MenuItems.AddRange(new System.Windows.Forms.MenuItem[] {
            this.menuItemIsBinary,
            this.menuItemIsBorderEmpty,
            this.menuItemIs4Connected});
            this.menuItemBinary.Text = "Binary";
            // 
            // menuItemIsBinary
            // 
            this.menuItemIsBinary.Index = 0;
            this.menuItemIsBinary.Text = "Binary?";
            this.menuItemIsBinary.Click += new System.EventHandler(this.CheckIsBinary);
            // 
            // menuItemIsBorderEmpty
            // 
            this.menuItemIsBorderEmpty.Index = 1;
            this.menuItemIsBorderEmpty.Text = "Border empty?";
            this.menuItemIsBorderEmpty.Click += new System.EventHandler(this.CheckIsBorderEmpty);
            // 
            // menuItemIs4Connected
            // 
            this.menuItemIs4Connected.Index = 2;
            this.menuItemIs4Connected.Text = "4-connected?";
            this.menuItemIs4Connected.Click += new System.EventHandler(this.Check4Connected);
            // 
            // CSImageViewer
            // 
            this.AllowDrop = true;
        ...
        protected override void OnPaint ( PaintEventArgs e ) {
            ...
            if (mMouseMoveValid) {
                ...
                mMouseX = (int)(mMouseX / Zoom);
                mMouseY = (int)(mMouseY / Zoom);

                string pixelValue = string.Empty;
                //report position 
                //calculate pixel
                if (mImage != null && mImage.mDisplayImage != null) { //when no image is opened
                     pixelValue = mImage.GetPixelValue(mMouseY, mMouseX );
                }
                //report position
                g.DrawString( "(" + mMouseX + "," + mMouseY + ")", mFont, mBrush1, 20, 40 );
                g.DrawString( "(" + mMouseX + "," + mMouseY + ")", mFont, mBrush2, 21, 41 );
                g.DrawString( "(" + pixelValue + ")", mFont, mBrush1, 20, 60 );
                g.DrawString( "(" + pixelValue + ")", mFont, mBrush2, 21, 61 );
            }
        }
        //--------------------------------------------------------------
        ...
        private void Check4Connected ( object sender, EventArgs e ) {

        }

        private void CheckIsBinary ( object sender, EventArgs e ) {

            CheckBinaryVisitor checkBinaryVisitor = new CheckBinaryVisitor();

            if (mImage != null && mImage.getIsColor() != true) {
                mGImage = (GrayImageData)mImage;

                checkBinaryVisitor.visit( mGImage );
                if(checkBinaryVisitor.get() == true) {
                    MessageBox.Show( "Image is Binary Image." + "\n max value: " + checkBinaryVisitor.getMax() + "\n min value: " + checkBinaryVisitor.getMin() );
                } else {
        ...
        private void CheckIsBorderEmpty ( object sender, EventArgs e ) {

        }

        private void OnKeyDown ( object sender, KeyEventArgs e ) {
            Console.WriteLine( "OnKeyDown" );
            if (mImage != null) {   //when no image is opened
                if (e.KeyCode == Keys.Add || e.KeyCode == Keys.Oemplus) {
                    if (Zoom <= 3) {
                        Zoom = Zoom + 0.1; //Distoration Method
                        this.Invalidate();  //cause repaint
                    }
