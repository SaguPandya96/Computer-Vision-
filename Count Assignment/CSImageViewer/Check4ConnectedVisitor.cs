    ...
    public class Check4ConnectedVisitor   //Public is define already 
    {
        ...
        public void visit ( GrayImageData g ) {
            ...
            if (max == min)
                ...
                if (cb.get() == true && cb1.get() == true) {       //if image is Binary and border empty then method will applied
                    for (int r = 1; r < row; r++) {
                        for (int c = 1; c < col; c++) {
                            if (g.getGray( r, c ) == min) {     //if value is min then label is 0 else assign new label here
                                L[ r, c ] = 0;
