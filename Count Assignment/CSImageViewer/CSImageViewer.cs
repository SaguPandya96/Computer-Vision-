...
namespace CSImageViewer {
    ...
    public class CSImageViewer : Form {
        ...
        private void InitializeComponent ( ) {
            ...
            this.menuItem5.Text = "Count objects";
            this.menuItem5.Click += new System.EventHandler( this.countObects );
            // 
            // CSImageViewer
            // 
            this.AllowDrop = true;
        ...
        private void countObects ( object sender, EventArgs e ) {
            CheckBinaryVisitor  b1=new  CheckBinaryVisitor();
            CheckBorderEmptyVisitor cbev = new  CheckBorderEmptyVisitor();
            Check4ConnectedVisitor  c1=new  Check4ConnectedVisitor();
            ...
            if (mImage != null && mImage.getIsColor() != true) {
                ...
                if (b1.get() == true) { //binary  image here
                    cbev.visit( mGImage );
                    if (cbev.get() == true) { //Check border empty
                        c1.visit( mGImage );
                        if (c1.get() == true) { //Check 4 connected
                            if (c1.getBadCount() == 0) {
                                countObjects.visit( mGImage );
                                MessageBox.Show( "Obect count: " + countObjects.get() + "\n Internal count: " + countObjects.getInternal() + "\n External count: " + countObjects.getExternal() );
                            } else {
                                MessageBox.Show( "Image has bad count: " + c1.getBadCount() );
                            }
                        } else
                            MessageBox.Show( "Image is not 4 connected" + "\n bad count: " + c1.getBadCount() + "\n bad pixel: (" + c1.getBadRow() + "," + c1.getBadCol() + ")" );
